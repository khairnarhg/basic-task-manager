...
//                 className={`bg-gray-800 p-4 rounded-lg shadow-inner transition-colors ${
          ...
          <Droppable droppableId={columnId} key={columnId}>
            {(provided, snapshot) => (
              <div
                ref={provided.innerRef}
                {...provided.droppableProps}
                className={`bg-gray-800 p-4 rounded-lg shadow-inner transition-colors ${snapshot.isDraggingOver ? 'bg-gray-700' : ''}`}
              >
                <h3 className="text-lg font-semibold mb-4 border-b-2 border-gray-600 pb-2">{column.name}</h3>
                
                {/* Scrollable container with padding */}
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-indigo-600 scrollbar-track-gray-700 hover:scrollbar-thumb-indigo-500">
                  {column.tasks.map((task, index) => (
                    <Draggable key={task.id} draggableId={task.id.toString()} index={index}>
                      {(provided, snapshot) => (
                        <div
                  ...
                  {provided.placeholder} {/* MUST be inside scrollable div */}
                </div>
              </div>
            )}
          </Droppable>
